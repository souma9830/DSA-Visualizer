...
import {
  Activity, BarChart3, Binary, CheckCheck, Clock3, Play, Pause, RotateCcw, 
  Code2, Shuffle, Eye, EyeOff, SlidersHorizontal, Layers, Copy, Gauge, 
  Keyboard, Radar, Sparkles, Download
} from 'lucide-react';
...
const CPP_KEYWORDS = new Set(['break', 'case', 'class', 'const', 'continue', 'default', 'do', 'else', 'enum', 'for', 'if', 'new', 'return', 'struct', 'switch', 'template', 'this', 'throw', 'typedef', 'using', 'virtual', 'while']);
const CPP_TYPES = new Set(['bool', 'char', 'double', 'float', 'int', 'long', 'short', 'void', 'string', 'vector', 'std']);
const CPP_TOKEN_REGEX = /\/\*[\s\S]*?\*\/|\/\/.*|"(?:\\.|[^"\\])*"|^\s*#.*$|\b\d+\b|\b[a-zA-Z_]\w*\b/gm;
...
const statusStyleMap = { Idle: 'border-white/15 bg-white/5 text-slate-200', Running: 'border-cyan-400/30 bg-cyan-500/10 text-cyan-100', Paused: 'border-amber-400/30 bg-amber-500/10 text-amber-100', Completed: 'border-emerald-400/30 bg-emerald-500/10 text-emerald-100', Unavailable: 'border-red-400/30 bg-red-500/10 text-red-100' };
const legendTemplate = [{ key: 'default', label: 'Default', status: 'default' }, { key: 'comparing', label: 'Comparing', status: 'comparing' }, { key: 'swapping', label: 'Swapping', status: 'swapping' }, { key: 'sorted', label: 'Sorted / Found', status: 'sorted' }, { key: 'special', label: 'Pivot / Target', status: 'pivot' }];
...
export default function VisualizerPage({ name, codeSnippet }) {
  ...
  const stageGlowClass = runStatus === 'Running' ? 'from-cyan-400/20 via-blue-500/10 to-transparent' : runStatus === 'Completed' ? 'from-emerald-400/20 via-cyan-500/10 to-transparent' : 'from-blue-500/12 via-slate-500/6 to-transparent';
  ...
  useEffect(() => {
    const handleHotkeys = (e) => {
      ...
      if (e.key.toLowerCase() === 'v') { e.preventDefault(); setShowValues(v => !v); }
  ...
  const handleStart = async () => {
    if (!algorithm?.run) { setRunStatus('Unavailable'); return; }
    ...
    if (stopSignal.current) { setIsSorting(false); setIsPaused(false); return; }
    if (!pauseSignal.current) { setIsSorting(false); setIsPaused(false); setRunStatus('Completed'); }
  };
    ...
    <div className="font-body relative mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:py-12">
      ...
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-800/40 p-5 shadow-2xl backdrop-blur sm:p-7">
        <div className="relative z-10 grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          <div>
            ...
            <h1 className="font-display text-3xl font-black text-white sm:text-4xl lg:text-5xl">{name}</h1>
            <p className="mt-3 text-sm text-slate-300 sm:text-base">{algorithm?.description}</p>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[360px_1fr]">
        ...
        <aside className="rounded-3xl border border-white/10 bg-slate-800/35 p-5 backdrop-blur xl:sticky xl:top-24">
          ...
          <div className="space-y-4">
            <div className="rounded-2xl bg-white/5 p-3">
              <label className="flex justify-between text-xs text-slate-400 mb-2 uppercase"><span><Layers size={14} className="inline mr-1"/> Size</span> <span>{arraySize}</span></label>
            ...
            <div className="rounded-2xl bg-white/5 p-3">
              <label className="flex justify-between text-xs text-slate-400 mb-2 uppercase"><span><Clock3 size={14} className="inline mr-1"/> Delay</span> <span>{speed}ms</span></label>
            ...
            <div className="grid grid-cols-2 gap-2">
              <MotionButton whileTap={{ scale: 0.95 }} onClick={handleResetHighlights} className="flex items-center justify-center gap-2 rounded-xl bg-white/5 py-2.5 text-sm font-bold text-white border border-white/10"><RotateCcw size={16}/> Reset</MotionButton>
              <MotionButton whileTap={{ scale: 0.95 }} onClick={() => handleGenerateNew()} className="flex items-center justify-center gap-2 rounded-xl bg-cyan-500/10 py-2.5 text-sm font-bold text-cyan-100 border border-cyan-400/20"><Shuffle size={16}/> New Data</MotionButton>
            </div>

            <div className="grid grid-cols-2 gap-2">
              <MotionButton onClick={() => setShowValues(!showValues)} className="flex items-center justify-center gap-2 rounded-xl bg-white/5 py-2.5 text-sm font-bold text-white border border-white/10">{showValues ? <EyeOff size={16}/> : <Eye size={16}/>} {showValues ? 'Hide' : 'Values'}</MotionButton>
              <MotionButton onClick={handleDownloadCode} className="flex items-center justify-center gap-2 rounded-xl bg-blue-500/10 py-2.5 text-sm font-bold text-blue-100 border border-blue-400/20"><Download size={16}/> Get C++</MotionButton>
            </div>

            {!isSorting ? (
              <MotionButton whileHover={{ scale: 1.02 }} onClick={handleStart} className="w-full flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-500 py-3.5 font-bold text-white shadow-lg"><Play size={18} fill="currentColor"/> Start</MotionButton>
            ) : (
              <MotionButton onClick={isPaused ? handleResume : handlePause} className={`w-full flex items-center justify-center gap-2 rounded-2xl py-3.5 font-bold text-white ${isPaused ? 'bg-emerald-600' : 'bg-amber-500 text-slate-900'}`}>{isPaused ? <Play size={18} fill="currentColor"/> : <Pause size={18} fill="currentColor"/>} {isPaused ? 'Resume' : 'Pause'}</MotionButton>
            )}
          </div>
        ...
        <section className="rounded-3xl border border-white/10 bg-slate-800/35 p-4 backdrop-blur sm:p-6 shadow-2xl">
          ...
          <div className="relative h-[300px] sm:h-[450px] bg-slate-900/55 rounded-2xl border border-slate-700/60 flex items-end justify-center gap-0.5 px-4 pb-4">
             ...
             {array.map((item, i) => (
                <MotionBar key={i} layout transition={{ type: 'spring', stiffness: 300, damping: 30 }} className={`relative rounded-t-sm ${getBarColor(item.status, themeColors)}`} style={{ height: `${(item.value / maxValue) * 100}%`, width: `${100/array.length}%` }}>
                  {canShowValues && <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-[9px] font-bold text-slate-400">{item.value}</span>}
                </MotionBar>
             ))}
          </div>
        </section>
      ...
      <section className="mt-6 overflow-hidden rounded-3xl border border-slate-800 bg-slate-950 shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-800 bg-slate-900 px-6 py-4">
          <div className="flex items-center gap-3"><Code2 size={20} className="text-blue-400"/><span className="text-sm font-bold uppercase tracking-widest text-slate-200">C++ Source</span></div>
          <div className="flex gap-2">
